...
export default function TrustStrip() {
  return (
    <section className="bg-[#1C1A17] py-24 md:py-32 overflow-hidden border-t border-stone-800">
            ...
            {trustData.ratings.map((rating) => (
              ...
              >
                <div className="font-serif text-5xl md:text-6xl font-bold text-white mb-2">
                  ...
                  <span className="text-2xl text-stone-500">/{rating.outOf}</span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {trustData.quotes.map((quote, i) => (
            <ScrollReveal key={i} delay={0.1 + i * 0.1}>
              <div className="glass-card-dark rounded-xl p-6 md:p-8 h-full flex flex-col border border-stone-850 bg-stone-900/60 shadow-lg">
                {/* Quote mark */}
                <svg
                  width="32"
                  height="24"
                  viewBox="0 0 32 24"
                  className="text-[#C5A880]/20 mb-4 shrink-0"
                  fill="currentColor"
                >
                ...
                <p className="text-stone-300 font-sans text-sm md:text-base leading-relaxed italic flex-1">
                  &ldquo;{quote.text}&rdquo;
                </p>
                <div className="mt-6 pt-4 border-t border-stone-800">
                  <p className="text-white font-medium text-sm font-sans">{quote.author}</p>
                  <p className="text-[#C5A880] text-xs font-sans">{quote.source}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
